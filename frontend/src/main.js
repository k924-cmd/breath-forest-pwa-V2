import { state, addLog, addMessage, saveState } from './app/state.js?v=20260808-25';
import { icon } from './components/icons.js?v=20260808-25';
import { homePage } from './pages/home.js?v=20260808-25';
import { devicesPage } from './pages/devices.js?v=20260808-25';
import { chatPage } from './pages/chat.js?v=20260808-25';
import { profilePage } from './pages/profile.js?v=20260808-25';
import { introPage, INTRO_SLOGAN, INTRO_SUBTITLE } from './components/intro.js?v=20260808-25';
import { loginPage } from './components/login.js?v=20260808-25';
import { login, isLoggedIn } from './auth/auth.js?v=20260808-25';
import { loadBackendSnapshot, sendConversationMessage, deleteMessages } from './services/conversation-service.js?v=20260808-25';
import { fetchWeather } from './services/weather-service.js?v=20260808-25';
import { toggleMockDevice } from './services/device-service.js?v=20260808-25';
import { getEnvironmentSnapshot } from './services/environment-service.js?v=20260808-25';
import { createMockDevices, findDevice, getDeviceMeta, normalizeBackendDevices } from './mocks/devices.js?v=20260808-25';
import { escapeHtml } from './utils/html.js?v=20260808-25';
import { AudioRecorder, supportsRecording, MAX_RECORD_MS } from './utils/audio.js?v=20260808-25';
import { initFeedback } from './utils/feedback.js?v=20260808-25';
import { transcribeAudio } from './services/asr-service.js?v=20260808-25';
import { synthesizeSpeech } from './services/tts-service.js?v=20260808-25';
import { playBase64Interruptible, stopPlayback, unlockAudio } from './utils/play-audio.js?v=20260808-25';
import { messageSignature, structuredMessageHtml } from './components/message-cards.js?v=20260808-25';
import {
  formatObservedAt,
  getDeviceStateLabel,
  getSourceLabel
} from './presentation.js?v=20260808-25';
...
async function speakReply(msgId, content) {
  ...
  const { synthesizeSpeechStream } = await import('./services/tts-service.js?v=20260808-25');
  const { pushStreamChunk, stopStreamPlayback } = await import('./utils/stream-playback.js?v=20260808-25');
...
async function startWakeWord() {
  const { startWake, wakeWordConfigured, setWakeLogger, WAKE_KEYWORD_LABEL } = await import('./wake/wake-service.js?v=20260808-25');
...
async function stopWakeWord() {
  const { stopWake } = await import('./wake/wake-service.js?v=20260808-25');
  wakeRunning = false;
  await stopWake();
}
...
// 唤醒确认播报：TTS 合成确认语并播放。与录音并发，失败静默降级；
// 受「语音播报」开关控制（关闭时唤醒确认不出声）。
async function speakWakeAck() {
  if (state.settings.speak !== true) return;
  try {
    const { playBase64Interruptible } = await import('./utils/play-audio.js?v=20260808-25');
    const { WAKE_ACK_TEXT } = await import('./config.js?v=20260808-25');
...
async function requestMicStreamForWake(recorder) {
  const { requestMicStream } = await import('./utils/mic-service.js?v=20260808-25');
...
function bindVoiceButton(button) {
  ...
  const finishRecording = async () => {
    ...
    // 复用已弹出的弹窗：先切到识别 loading 态，转写完成后再填充
    const modalId = recorder.__voiceModalId || showVoiceConfirmModal(null, 'loading');
    updateVoiceConfirmModal(modalId, null, null, 'loading');
    const result = await transcribeAudio(blob);
    recorder = null;
...
// 语音转写弹窗：phase='recording' 长按中正在聆听；phase='loading' 识别中；
// phase=null 且 text 有值 → 直接展示完成态。识别完成后 updateVoiceConfirmModal 填充。
function showVoiceConfirmModal(text, phase = 'loading') {
  const modalRoot = document.querySelector('#modal-root');
  if (!modalRoot) return null;
  const modalId = `voice-confirm-${Date.now()}`;
  const isPending = text == null;
  let pendingHtml = '';
  if (isPending && phase === 'recording') {
    pendingHtml = `<div class="voice-listening"><div class="voice-wave"><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div><p class="voice-loading-text">正在聆听，请说话…</p></div>`;
  } else if (isPending) {
    pendingHtml = `<div class="voice-loading"><div class="voice-loading-orbs"><i></i><i></i><i></i></div><p class="voice-loading-text">正在竖起耳朵听你说…</p></div>`;
  }
  modalRoot.innerHTML = `<div class="modal voice-confirm-modal" data-voice-modal="${modalId}"><section class="voice-confirm-sheet" role="dialog" aria-modal="true"><header><span class="voice-confirm-icon">${icon('mic')}</span><div><span class="eyebrow">VOICE INPUT</span><h2>${isPending ? '聆听你的声音' : '确认语音内容'}</h2></div><button data-action="close" aria-label="取消">×</button></header><div class="voice-confirm-body">${pendingHtml}<textarea id="voice-confirm-text" rows="3" maxlength="4000" aria-label="语音识别文字" ${text == null ? 'hidden' : ''}>${text == null ? '' : escapeHtml(text)}</textarea></div><div class="voice-confirm-actions"><button class="voice-cancel" data-action="voice-cancel" ${text == null ? 'disabled' : ''}>取消</button><button class="voice-send" data-action="voice-send" ${text == null ? 'disabled' : ''}>发送</button></div></section></div>`;
  if (text != null) {
...
function updateVoiceConfirmModal(modalId, text, error, phase) {
  ...
  if (!body) return;
  // 录音中 → 识别中：切换 body 为 loading 泡泡
  if (phase === 'loading') {
    const h2 = modal.querySelector('h2');
    if (h2) h2.textContent = '正在识别…';
    body.innerHTML = `<div class="voice-loading"><div class="voice-loading-orbs"><i></i><i></i><i></i></div><p class="voice-loading-text">正在竖起耳朵听你说…</p></div>`;
    const textEl = body.querySelector('.voice-loading-text');
    if (textEl) {
      let index = 0;
      const tick = () => {
        index = (index + 1) % VOICE_LOADING_TEXTS.length;
